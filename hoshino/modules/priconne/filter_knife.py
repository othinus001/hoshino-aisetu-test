...
if not yobot_url:
    yobot_url = 'http://bot.uedbq.com:9222/' 
    # 获取主页地址：在群内向bot发送指令“手册”，复制bot发送的链接地址，删除末尾的manual/后即为主页地址
    # 例:https://域名/目录/或http://IP地址:端口号/目录/,注意不要漏掉最后的斜杠！

DB_PATH = get_db_path()
if not DB_PATH:
    DB_PATH = 'C:/Users/Administrator/Desktop/xcw/HoshinoBot/hoshino/modules/yobot/yobot/src/client/yobot_data/yobotdata.db'
...
async def _ (bot, ev):
    ...
    for m in ev['message']:
        ...
        # 检查消息有没有带@信息，有的话uid改为@的QQ号
            uid = str(m['data']['qq'])
            at_name = await bot.get_group_member_info(group_id=gid,user_id=uid)
            name = f"{at_name['nickname']}({str(ev['sender']['card'])} 代报)"
            print(name)
...
async def _ (bot, ev):
    ...
    for m in ev['message']:
        ...
        # 检查消息有没有带@信息，有的话uid改为@的QQ号
            uid = str(m['data']['qq'])
    if (gid in filter_knife_data) and (uid not in filter_knife_data[gid]):
        await bot.send(ev,"之前未有筛刀记录，请发送[筛刀 伤害]新增")
        return
    if gid not in filter_knife_data:
        await bot.send(ev, "之前未有筛刀记录，请发送[筛刀 伤害]新增")
        return
    filter_knife_data[gid][uid]["damage"] = damage
